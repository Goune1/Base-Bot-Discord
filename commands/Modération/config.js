...
module.exports= {
    ...
    run: async(Client, message, args, config, bienvenue_statut, logs_statut, roleBienvenue_statut) => { 
       ...
       /* else if(configStatut.content == "suggest") {
            message.reply("Voulez vous activer le système de suggestion (`on`), l'éteindre (`off`) ou changer le salon (`setup`) ?")
            ...
            if(configSuggest.content == "on") {
                ...
                Savebdd();
                return message.channel.send(`Le système de suggestion est désormais activé !`)
            }
            if(configSuggest.content == "off"){
                config["statut-suggest"] = false
                Savebdd(); 
                return message.channel.send(`Le système de suggestion est désormais désactivé !`) 
            }
            if(configSuggest.content == "setup") {
                message.channel.send("Quel est l'identifiant du nouveau salon pour les suggestions ?")
                ...
                Savebdd()
                return message.channel.send(`L'identifiant du salon des suggestions est maintenant : **${suggestChannel.content}**`)
