...
module.exports= {
    ...
    run: async(Client, message, args, config, token, eco) => { 
        ...
        if(somme > eco[message.author.id]["bank"]) return message.channel.send("Vous ne pouvez pas transférer plus que ce que vous avez !")
        if(isNaN(somme)) return message.channel.send("Veuillez indiquer un nombre de dollars à transférer !")

        eco[message.author.id]["bank"] = eco[message.author.id]["bank"] - somme
